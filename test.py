...
def show_nova_ia_page():
    ...
    if st.session_state["nova_ia_phase"] in ("dialogue",):
        ...
        if envoyer and msg_input.strip():
            ...
            # ── DÉTECTION REPRODUCTION IMAGE → .docx direct ──────────────
            _fichier_repro = st.session_state.get("nova_ia_fichier")
            _mots_repro = ["reprodui", "reproduc", "copie", "copier", "identique", "à l'identique",
                           "word", "docx", "tableau", "recopie", "recopier", "même chose", "pareil"]
            _est_demande_repro = (
                _fichier_repro is not None
                and _fichier_repro.get("est_image")
                and any(m in msg_input.lower() for m in _mots_repro)
            )

            if _est_demande_repro:
                st.session_state["nova_ia_fichier"] = None
                with st.spinner("🔍 Nova IA analyse l'image et reconstruit le document..."):
                    import json as _json

                    _prompt_repro = (
                        "Tu reçois une image contenant un document (tableau, liste, formulaire, texte, ou tout autre contenu).\n"
                        "TON SEUL RÔLE : extraire EXACTEMENT le contenu visible, sans rien inventer, modifier ou ajouter.\n\n"
                        "RÈGLES STRICTES :\n"
                        "- Copie UNIQUEMENT ce qui est visible dans l'image. Zéro ajout, zéro reformulation.\n"
                        "- Si tu vois un titre, copie-le mot pour mot.\n"
                        "- Si tu vois un tableau, extrais toutes les colonnes et lignes exactement comme elles apparaissent.\n"
                        "- Si tu vois du texte libre, copie-le tel quel.\n"
                        "- INTERDIT : inventer des données, compléter des cellules vides, ajouter une introduction ou conclusion.\n\n"
                        "Réponds UNIQUEMENT en JSON valide, sans aucun texte avant ou après, sans balises markdown.\n"
                        "Format attendu :\n"
                        "{\n"
                        "  \"titre\": \"Titre du document si visible, sinon null\",\n"
                        "  \"sous_titre\": \"Sous-titre ou mention si visible, sinon null\",\n"
                        "  \"en_tete\": \"Texte d'en-tête (ex: date, organisme) si visible, sinon null\",\n"
                        "  \"type\": \"tableau\" ou \"texte\" ou \"liste\" ou \"mixte\",\n"
                        "  \"colonnes\": [\"Colonne1\", \"Colonne2\", ...],\n"
                        "  \"lignes\": [[\"val1\", \"val2\", ...], [\"val1\", \"val2\", ...], ...],\n"
                        "  \"texte_libre\": \"Si pas de tableau, le texte brut ici, sinon null\"\n"
                        "}"
                    )

                    _reponse_json_brut = generer_avec_gemini(
                        "Reproduction Image Word",
                        _prompt_repro,
                        user or "visiteur",
                        _image_b64=_fichier_repro["contenu"],
                        _image_mime=_fichier_repro["type"]
                    )
                ...
                _repro_data = None
                try:
                    _clean = _reponse_json_brut.strip()
                    if _clean.startswith("```"):
                        _clean = _clean.split("```")[1]
                        if _clean.startswith("json"):
                            _clean = _clean[4:]
                    _clean = _clean.strip()
                    _repro_data = _json.loads(_clean)
                    _repro_ok = True
                except Exception:
                    pass

                if _repro_ok and _repro_data:
                    # ── Construire le .docx fidèle à l'image ──────────────
                    from docx import Document as _DocRepro
                    ...
                    import io as _io_repro

                    _doc_repro = _DocRepro()

                    # Marges serrées
                    for sec in _doc_repro.sections:
                        sec.top_margin    = _Inches(0.7)
                        sec.bottom_margin = _Inches(0.7)
                        sec.left_margin   = _Inches(0.8)
                        sec.right_margin  = _Inches(0.8)

                    # En-tête (organisme, date, etc.)
                    if _repro_data.get("en_tete"):
                        _p_ent = _doc_repro.add_paragraph()
                        _p_ent.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        _run_ent = _p_ent.add_run(_repro_data["en_tete"])
                        _run_ent.font.size = _Pt(10)

                    # Titre principal
                    if _repro_data.get("titre"):
                        _p_tit = _doc_repro.add_paragraph()
                        _p_tit.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        _run_tit = _p_tit.add_run(_repro_data["titre"])
                        _run_tit.bold = True
                        _run_tit.font.size = _Pt(16)
                        _run_tit.font.color.rgb = _RGB(0x1F, 0x49, 0x99)

                    # Sous-titre
                    if _repro_data.get("sous_titre"):
                        _p_sub = _doc_repro.add_paragraph()
                        _p_sub.alignment = WD_ALIGN_PARAGRAPH.CENTER
                        _run_sub = _p_sub.add_run(_repro_data["sous_titre"])
                        _run_sub.font.size = _Pt(11)

                    _doc_repro.add_paragraph()

                    # Tableau
                    if _repro_data.get("type") == "tableau" and _repro_data.get("colonnes") and _repro_data.get("lignes"):
                        _cols = _repro_data["colonnes"]
                        _rows = _repro_data["lignes"]
                        _nb_cols = len(_cols)
                        _nb_rows = len(_rows)

                        _tbl = _doc_repro.add_table(rows=1 + _nb_rows, cols=_nb_cols)
                        _tbl.style = "Table Grid"

                        # En-tête du tableau
                        _hdr_cells = _tbl.rows[0].cells
                        for i, col_name in enumerate(_cols):
                            _hdr_cells[i].text = str(col_name)
                            _run_h = _hdr_cells[i].paragraphs[0].runs[0]
                            _run_h.bold = True
                            _run_h.font.size = _Pt(10)
                            # Fond gris clair pour l'en-tête
                            _tc = _hdr_cells[i]._tc
                            _tcPr = _tc.get_or_add_tcPr()
                            _shd = _OxmlEl("w:shd")
                            _shd.set(_qn("w:val"), "clear")
                            _shd.set(_qn("w:color"), "auto")
                            _shd.set(_qn("w:fill"), "D9D9D9")
                            _tcPr.append(_shd)

                        # Données
                        for r_idx, row_data in enumerate(_rows):
                            _data_cells = _tbl.rows[r_idx + 1].cells
                            for c_idx, val in enumerate(row_data):
                                if c_idx < _nb_cols:
                                    _data_cells[c_idx].text = str(val) if val is not None else ""
                                    _data_cells[c_idx].paragraphs[0].runs[0].font.size = _Pt(10)

                    elif _repro_data.get("texte_libre"):
                        _p_txt = _doc_repro.add_paragraph(_repro_data["texte_libre"])
                        _p_txt.runs[0].font.size = _Pt(11)

                    # Sauvegarder
                    _buf_repro = _io_repro.BytesIO()
                    _doc_repro.save(_buf_repro)
                    _buf_repro.seek(0)

                    _nom_repro = f"Nova_Reproduction_{datetime.now().strftime('%d%m%Y_%H%M')}.docx"

                    # Upload Supabase si connecté
                    import uuid as _uuid_repro
                    _req_repro = str(_uuid_repro.uuid4())[:8]
                    try:
                        _url_repro = upload_fichier_client(user or "visiteur", _req_repro, _io_repro.BytesIO(_buf_repro.read()), _nom_repro)
                        _buf_repro.seek(0)
                        if _url_repro and not _url_repro.startswith("ERREUR"):
                            save_lien(user or "visiteur", "📄 Reproduction Image Word", _url_repro, datetime.now().strftime("%d/%m/%Y"))
                    except Exception:
                        pass
                    ...
                    st.session_state["nova_ia_livrable"] = {
                        "buf": _buf_repro.read(),
                        "nom": _nom_repro,
                        "service": "Reproduction Image Word",
                        "url": None,
                    }

                    _msg_repro_ok = "✅ Document reproduit fidèlement ! Télécharge-le directement ici 👇"
                    st.session_state["nova_ia_chat"].append({"role": "assistant", "content": _msg_repro_ok})
                    st.session_state["nova_ia_phase"] = "termine"
                    st.rerun()

                else:
                    # Échec parsing JSON → message d'erreur dans le chat
                    _msg_repro_err = "⚠️ Je n'ai pas réussi à analyser correctement l'image. Essaie avec une image plus nette ou décris moi le contenu directement."
                    st.session_state["nova_ia_chat"].append({"role": "assistant", "content": _msg_repro_err})
                    st.rerun()

            # ── FIN BLOC REPRODUCTION IMAGE ───────────────────────────────
