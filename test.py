...
def main_dashboard():
    ...
    if st.session_state.get("show_mode_modal", False):
        ...
        # ── CSS : cache UI Streamlit + style boutons ──────────────
        st.markdown("""
        <style>
        ...
        .nm-halo-c { width:600px;height:600px;bottom:-200px;right:-180px;
            ...
            animation:pulseHalo 7s ease-in-out infinite 1s; }
        .nm-halo-m { width:400px;height:400px;top:40%;left:40%;
            background:radial-gradient(circle,rgba(160,80,255,.07) 0%,transparent 70%);
            animation:pulseHalo 9s ease-in-out infinite 2s; }
        @keyframes pulseHalo {
        ...
        .nm-ptc:nth-child(1)  { left:10%; background:#FFD700; animation-delay:0s;   animation-duration:7s; }
        .nm-ptc:nth-child(2)  { left:25%; background:#00d2ff; animation-delay:1s;   animation-duration:9s; }
        .nm-ptc:nth-child(3)  { left:40%; background:#c87aff; animation-delay:2s;   animation-duration:6s; }
        .nm-ptc:nth-child(4)  { left:55%; background:#FFD700; animation-delay:3s;   animation-duration:8s; }
        .nm-ptc:nth-child(5)  { left:70%; background:#00d2ff; animation-delay:0.5s; animation-duration:10s; }
        .nm-ptc:nth-child(6)  { left:85%; background:#c87aff; animation-delay:1.5s; animation-duration:7s; }
        .nm-ptc:nth-child(7)  { left:18%; background:#FFD700; animation-delay:4s;   animation-duration:8s; }
        .nm-ptc:nth-child(8)  { left:62%; background:#00d2ff; animation-delay:2.5s; animation-duration:9s; }
        .nm-ptc:nth-child(9)  { left:90%; background:#c87aff; animation-delay:3.5s; animation-duration:6s; }
        @keyframes floatUp {
        ...
        .nm-header {
            ...
            animation:dropIn .8s cubic-bezier(.22,1,.36,1) both;
        }
        @keyframes dropIn {
        ...
        .nm-logo-ring1 {
            ...
            border:1.5px dashed rgba(255,215,0,.5);
            animation:spinCW 8s linear infinite;
        }
        .nm-logo-ring2 {
            position:absolute; inset:-14px; border-radius:50%;
            border:1px dashed rgba(0,210,255,.35);
            animation:spinCCW 12s linear infinite;
        }
        @keyframes spinCW  { to { transform:rotate(360deg);  } }
        @keyframes spinCCW { to { transform:rotate(-360deg); } }
        .nm-logo-icon { font-size:2.2rem; position:relative; z-index:2;
            filter:drop-shadow(0 0 14px #FFD700); }
        .nm-title {
            font-size:clamp(1.6rem,4vw,2.4rem); font-weight:900; letter-spacing:6px;
            color:#fff; text-shadow:0 0 40px rgba(255,215,0,.5);
            background:linear-gradient(90deg,#FFD700,#fff,#00d2ff);
            -webkit-background-clip:text; -webkit-text-fill-color:transparent;
            background-clip:text;
        }
        .nm-sub {
            font-size:.75rem; letter-spacing:3px; color:rgba(255,255,255,.35);
        ...
        .nm-divline-d { width:5px; height:5px; border-radius:50%;
            background:#FFD700; animation:tagWave 2s ease-in-out infinite; }
        @keyframes tagWave {
            0%,100% { transform:translateY(0); }
        ...
        .nm-vs {
            display:flex; flex-direction:column;
            align-items:center; justify-content:center;
            gap:10px; flex-shrink:0; padding:0 18px;
        }
        .nm-vs-line {
            width:1px; flex:1;
            background:linear-gradient(180deg,transparent,rgba(255,255,255,.12),rgba(255,255,255,.18),rgba(255,255,255,.12),transparent);
        ...
        @keyframes vsGlow {
            0%,100% { text-shadow:0 0 8px rgba(255,255,255,.15); color:rgba(255,255,255,.2); }
            50%      { text-shadow:0 0 22px rgba(255,255,255,.5); color:rgba(255,255,255,.45); }
        }
        ...
        .nm-card:hover { transform:translateY(-8px) scale(1.018); }
        .nm-card-gold {
        ...
        .nm-card-cyan {
            ...
            animation:cardBreathCyan 4.5s ease-in-out infinite 1s, slideRight .75s cubic-bezier(.22,1,.36,1) .1s both;
        }
        ...
        .nm-corner {
            ...
            border-color:rgba(255,215,0,.4); border-style:solid;
            pointer-events:none; transition:border-color .3s;
        }
        .nm-card-cyan .nm-corner { border-color:rgba(0,210,255,.4); }
        .nm-card:hover .nm-corner { border-color:rgba(255,255,255,.6); }
        ...
        .nm-card-cyan .nm-spark { color:rgba(0,210,255,.7); }
        .nm-spark:nth-child(1) { top:18%; left:12%; animation-delay:0s; }
        .nm-spark:nth-child(2) { top:55%; right:10%; animation-delay:1.2s; }
        .nm-spark:nth-child(3) { bottom:22%; left:20%; animation-delay:2.4s; }
        @keyframes sparkle {
        ...
        .nm-icon { font-size:2.2rem; position:relative; z-index:2;
            filter:drop-shadow(0 0 14px currentColor); }
        .nm-icon-gold { color:#FFD700; filter:drop-shadow(0 0 14px rgba(255,215,0,.8)); }
        .nm-icon-cyan { color:#00d2ff; filter:drop-shadow(0 0 14px rgba(0,210,255,.8)); }
        .nm-icon-ring1 {
            position:absolute; inset:-8px; border-radius:50%;
            border:1.5px dashed rgba(255,215,0,.6);
        ...
        .nm-card-cyan .nm-icon-ring1 { border-color:rgba(0,210,255,.6); }
        .nm-card-cyan .nm-icon-ring2 { border-color:rgba(0,210,255,.3); }

        /* ─── BADGE ─── */
        .nm-badge {
            display:inline-block; border-radius:100px;
            padding:5px 16px; font-size:.68rem; font-weight:700;
            letter-spacing:2px; margin-bottom:12px;
            animation:badgePop .6s cubic-bezier(.34,1.56,.64,1) .5s both;
        }
        .nm-badge-g { background:rgba(255,215,0,.12); border:1px solid rgba(255,215,0,.5); color:#FFD700; }
        .nm-badge-c { background:rgba(0,210,255,.12); border:1px solid rgba(0,210,255,.5); color:#00d2ff; }
        @keyframes badgePop {
        ...
        .nm-card-cyan .nm-card-title { text-shadow:0 0 20px rgba(0,210,255,.4); }
        ...
        /* ─── TAGS ─── */
        .nm-tags { display:flex; flex-wrap:wrap; gap:6px; justify-content:center; margin-bottom:20px; }
        .nm-tag {
            padding:4px 12px; border-radius:100px; font-size:.68rem;
            font-weight:600; letter-spacing:.5px;
            animation:tagWave 2.5s ease-in-out infinite;
        }
        .nm-tag-g { background:rgba(255,215,0,.10); border:1px solid rgba(255,215,0,.35); color:rgba(255,215,0,.85); }
        .nm-tag-c { background:rgba(0,210,255,.10); border:1px solid rgba(0,210,255,.35); color:rgba(0,210,255,.85); }
        .nm-tag:nth-child(2) { animation-delay:.4s; }
        .nm-tag:nth-child(3) { animation-delay:.8s; }

        /* ─── BOUTONS STREAMLIT STYLISÉS ─── */
        [data-testid="stButton"][data-key="nm_btn_platform"] button,
        [data-testid="stButton"][data-key="nm_btn_ia"] button {
        ...
        [data-testid="stButton"][data-key="nm_btn_platform"] button {
            ...
            background-size:200% auto;
            color:#000;
            box-shadow:0 6px 28px rgba(255,180,0,.45);
        ...
        [data-testid="stButton"][data-key="nm_btn_ia"] button {
            ...
            background-size:200% auto;
            color:#fff;
            box-shadow:0 6px 28px rgba(0,180,255,.45);
            animation:btnShimmer 3s linear infinite;
        }
        ...
        [data-testid="stButton"][data-key="nm_btn_platform"] button:hover,
        [data-testid="stButton"][data-key="nm_btn_ia"] button:hover {
        ...
        .nm-footer-dot { display:inline-block; width:3px; height:3px; border-radius:50%;
            background:rgba(255,255,255,.25); margin:0 8px; vertical-align:middle; }
        </style>
        """, unsafe_allow_html=True)

        # ── FOND + HALOS + PARTICULES ──────────────────────────────
        st.markdown("""
        <div class="nm-bg">
        ...
        # ── CARTES SIDE BY SIDE via colonnes Streamlit ────────────
        ...
        with col_gold:
            ...
            """, unsafe_allow_html=True)
            if st.button("🖥️  UTILISER LA PLATEFORME", key="nm_btn_platform", use_container_width=True):
                st.session_state["show_mode_modal"] = False
                st.rerun()

        with col_vs:
            st.markdown("""
            <div class="nm-vs" style="height:100%;">
        ...
        with col_cyan:
            ...
            """, unsafe_allow_html=True)
            if st.button("🤖  DISCUTER AVEC NOVA IA", key="nm_btn_ia", use_container_width=True):
                st.session_state["show_mode_modal"] = False
